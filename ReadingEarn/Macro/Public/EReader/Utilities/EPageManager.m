...
- (void)initAndOpenReader:(NSString *)chapter_number {
    ...
    EEveryChapter *chapter = [[EDataSourceManager shareInstance] openChapters:chapter_number];
    [self parseChapter:chapter];
}
...
//        }
        if ([chapter.locked isEqualToString:@"1"] && ![[EDataSourceManager shareInstance] isVipStatus]) {
            //textController.showType = EShowNotFreeType;
        }
    }
