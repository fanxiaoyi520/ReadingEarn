...
- (void)viewDidLoad {
    ...
    [super viewDidLoad];
//    REReadingEarnPopupView *popView = [REReadingEarnPopupView readingEarnPopupViewWithType:REReadingEarnPopupView_ImmediateRecharge];
//    [popView showPopupViewWithData:@""];
//    __weak typeof (self)weekself = self;
//    popView.immediateRechargeBlock = ^(UIButton * _Nonnull sender) {
...
//    };
    ...
    if (self.chapter_number == nil || [self.chapter_number isEqual:[NSNull null]]){
        self.chapter_number = @"";
    }
    ...
    [[EDataSourceManager shareInstance] downloadChapterInfo:self.bookId withImageStr:self.imageStr withPage:self.page finish:^{
        [weakSelf.loadingView dismiss];
        [weakSelf.ePageManager initAndOpenReader:self.chapter_number];
...
- (void)settingTopBar:(ESettingTopBar *)settingTopBar actionGoBack:(UIButton *)button{
    ...
    [self searchNovel];
    [[EDataSourceManager shareInstance] removeDataBase:@"txt"];
...
- (DefaultServerRequest *)requestA {
    ...
    NSString *chapter_number = [NSString stringWithFormat:@"%ld",[EDataSourceManager shareInstance].everyChapter.chapterNum];
    if (!_requestA) {
        ...
        _requestA.requestURI = [NSString stringWithFormat:@"%@/novel/set_read_book",HomePageDomainName];
        _requestA.requestParameter = @{
                                       @"bookid":self.bookId,
                                       @"token":token,
                                       @"chaps":chapter_number,
                                      };
    }
    return _requestA;
}
...
//    NSLog(@"%ld",(long)[EDataSourceManager shareInstance].everyChapter.next);
//    [self turnToClickChapter:[EDataSourceManager shareInstance].everyChapter.next];
...
- (void)settingBottomBar:(ESettingBottomBar *)settingBottomBar turnToPreChapter:(UIButton *)button{//上一章
    ...
    //NSLog(@"%ld",(long)[EDataSourceManager shareInstance].everyChapter.last);
    if ([EDataSourceManager shareInstance].currentChapterIndex <= 1) {
        [EHUDView showMsg:@"已经是第一章" inView:self.view];
        return;
    }
    //[self turnToClickChapter:[EDataSourceManager shareInstance].everyChapter.last];
    [self turnToClickChapter:[EDataSourceManager shareInstance].currentChapterIndex - 1];
...
- (void)leftDrawerView:(ELeftDrawerView *)leftDrawerView turnfooterWithRefreshing:(NSString *)str {
    self.page++;
    WS(weakSelf);
    [[EDataSourceManager shareInstance] downloadChapterInfo:self.bookId withImageStr:self.imageStr withPage:self.page finish:^{
        [weakSelf.loadingView dismiss];
        ...
        //[weakSelf initPageView:[[EDataSourceManager shareInstance] openPage]];
        [weakSelf.leftDrawerView.listView.listView reloadData];
        [weakSelf.leftDrawerView.listView.listView.mj_footer  endRefreshing];
        [EDataSourceManager shareInstance].readingBlock = ^(NSInteger currentChapter){
