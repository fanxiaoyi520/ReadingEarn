...
@interface REMyNewsViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation REMyNewsViewController
    ...
    [self.requestA startWithSuccess:^(YCNetworkResponse * _Nonnull response) {
        ...
        if ([code isEqualToString:@"1"]) {
            ...
            for (NSDictionary *dic in array) {
                REMyNewsModel *model = [REMyNewsModel mj_objectWithKeyValues:dic];
...
- (DefaultServerRequest *)requestA {
    ...
    if (!_requestA) {
        ...
        _requestA.requestURI = [NSString stringWithFormat:@"%@/message/message_list",HomePageDomainName];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    REMyNewsModel *model = self.personalArray[indexPath.row];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    REMyNewsDetailViewController *vc = [REMyNewsDetailViewController new];
    vc.naviTitle = @"消息详情";
    REMyNewsModel *model = self.personalArray[indexPath.row];
